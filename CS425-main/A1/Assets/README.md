...
def simulate_client(username, password):
    try:
        ...
        # Hold connection open to guarantee concurrency
        time.sleep(15)
...
__5. Memory Garbage in send_all__
   - **Problem**: The `send_all` function was unconditionally sending `BUFFER_SIZE` (1024) bytes over the socket regardless of the actual message size. This transmitted uninitialized memory (garbage) which caused decoding crashes during stress testing.
   - **Solution**: Refactored `send_all` to calculate `bytes_to_send = min(length - total_sent, BUFFER_SIZE)` to only send the precise message length.
